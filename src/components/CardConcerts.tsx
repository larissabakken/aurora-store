...
const CardConcerts: React.FC<Props> = ({
  ...
  const ulrAurora = "https://www.aurora-music.com/";
  const ticketStore = () => {
    window.open(ulrAurora, "_blank");
  };
  return (
    <div className="bg-card-bg-color my-4 mx-auto sm:mx-10 py-2 rounded-sm flex items-center justify-between px-2">
      ...
      <button
        onClick={ticketStore}
        className="w-1/4 truncate px-4 py-2 border bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] text-white rounded-md shadow-2xl hover:shadow-hover-color hover:scale-110"
      >
        Tickets to buy
      </button>
    </div>
